...
class PSO:
    def __init__(self, parameters):
        ...
        self.bound = []                 # 变量的约束范围
        self.bound.append(parameters[2])    #最小
        self.bound.append(parameters[3])    #最大
        ...
        for i in range(self.pop_size):
            for j in range(self.var_num):
                self.pop_x[i][j] = random.uniform(self.bound[0][j], self.bound[1][j])   #[min, max]
                self.pop_v[i][j] = random.uniform(0, 1)

            self.p_best[i] = self.pop_x[i]      #储存最优的个体
            fit = self.fitness(self.p_best[i])  #返回一个值
            if fit > temp:
