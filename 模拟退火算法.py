...
class SA:
    def __init__(self, func, iter=100, T0=100, Tf=0.01, alpha=0.99):
        ...
        （random() * 10 -5）的结果是-5到5之间的任意值（不包括-5和5），所有先乘以11，取-6到6之间的值，产生新解过程中，
        用一个if条件语句把-5到5之间（包括整数5和-5）的筛选出来。
        """
        self.history = {'f': [], 'T': []}
    ...
    def run(self):
        ...
        print(f"F={f_best}, x={self.x[idx]}, y={self.y[idx]}, func={func(self.x[idx],self.y[idx])}")
